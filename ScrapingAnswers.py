...
#c.execute("CREATE TABLE AptiAnswers(SectionId integer, QuestionId integer, Answer text)")
...
for i in range (0,35):
    for j in range (1,list3[i]+1):
        ...
        for m in soup.find_all(class_="jq-hdnakqb mx-bold"):
            c.execute("INSERT INTO AptiAnswers VALUES(?,?,?);",(i+1,k,m.get_text()))
            k=k+1
            conn.commit()
    ...
    ''' print(m.get_text(),k)
        ...
        #print(("http://www.indiabix.com/aptitude/"+list1[i]+list2[i]+str(j)))

c.execute("SELECT * FROM AptiAnswers")
data=c.fetchall()
