...
c.execute("CREATE TABLE AptiQuestions(SectionId integer,QuestionId integer,Question text)")
...
for i in range (0,35):
    for j in range (1,list3[i]+1):
        ...
        for m in soup.find_all(class_="bix-td-qtxt"):
            c.execute("INSERT INTO AptiQuestions VALUES(?,?,?);",(i+1,k,m.get_text().strip(),))
            conn.commit()
            k=k+1
